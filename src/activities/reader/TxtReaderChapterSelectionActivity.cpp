...
void TxtReaderChapterSelectionActivity::render(RenderLock&&) {
  ...
  // Chapter titles are CJK on SD-card fonts: scan pass accumulates the glyphs,
  // prewarm batch-loads their bitmaps, then the real pass draws from cache.
  // Without this, every list redraw re-reads evicted glyphs from the SD card.
  // The scope must stay alive through the real draw (its destructor clears the
  // mini cache).
  auto* fcm = renderer.getFontCacheManager();
  if (fcm) {
    auto scope = fcm->createPrewarmScope();
    drawContent();  // scan pass — text accumulated, no drawing
    scope.endScanAndPrewarm();
    drawContent();  // real pass
