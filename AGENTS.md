...
- Anti-Hallucination: Do not assume the existence of libraries or ESP-IDF functions. If you are unsure of an API's availability for the ESP32-S3 Xtensa target, check the freeink-sdk source or the FreeInk SDK docs (https://freeink.org/llms.txt for an LLM-readable index) first.
...
- **Linux/MacOS/WSL**: Full bash, Unix paths, native glob support
...
- MCU: ESP32-S3 (Single-core Xtensa @ 240MHz)
- RAM: ~512KB DRAM + 8MB OPI-PSRAM
  - **PSRAM**: Large buffers (framebuffer, image decoding) should use PSRAM via `ps_malloc()` or `MALLOC_CAP_SPIRAM`
...
- Display: 960x540 E-Ink (parallel bus via IT8951-like driver, capacitive touch via GT911)
  - Framebuffer: 518,400 bytes for 960x540 8bpp
...
1. Stack Safety: Limit local function variables to < 512 bytes. The ESP32-S3 default stack is small; use std::unique_ptr or static pools for larger buffers.
...
2. **CLI Tool** (`pio` command):
   ...
   - **MacOS/Linux Location**: `/usr/local/bin/pio` or `~/.platformio/penv/bin/pio`
...
- lib/: Internal libraries (Epub engine, GfxRenderer, UITheme, I18n)
  - lib/hal/: Hardware Abstraction Layer (HalDisplay, HalGPIO, HalStorage, HalTouch, HalPowerManager)
...
**Button Categories** (Need to Porting to TOUCH):
